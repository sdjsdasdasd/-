...
# 读取Excel数据
data = pd.read_excel('SingleDay.xlsx')
...
for _, row in data.iterrows():
    start_time = datetime.strptime(row['充电开始时间'], time_format)
    end_time = datetime.strptime(row['充电结束时间'], time_format)
...
# 如果需要保存为Excel文件
pd.DataFrame(power_array, columns=['功率']).to_excel('power_curve.xlsx', index=False)
